...
\updated{This concern is especially urgent due to the interdependence between temperature and the leakage power \cite{srivastava2010, liu2007}.}
...
\updated{The result, labeled as ``Nominal'', is depicted in \fref{motivation-temperature} where, for clarity, only one curve, corresponding to one processor, is presented.}
...
\updated{The result is the ``Mild'' curve in \fref{motivation-temperature}; the maximal temperature is approaching $100^{\circ}$C.
...
\updated{In this case, even with mild deviations, the circuits might be burnt.
Another path that the designer could take is to design the system for severe conditions.
However, in this scenario, the system can easily end up being too conservative and over-designed.
...
\updated{A summary of the main notations is given in \tref{notation}.}
