...
$\ePDF$ is chosen as it reflects the quality of the distributions estimated by our framework, and it is computed by sampling the constructed PC expansions, which have negligible overheads (see \sref{post-processing}).
...
Another important observation regarding MC sampling is that, for a fixed $\pcorder \geq 4$, $\eVar$ exhibits a considerable decrease in \tref{accuracy-eta-0-5} even when $\nsamples$ transitions from $10^4$ to $10^5$.
...
Lastly, we would like mention that the aforementioned conclusions, based on \tref{accuracy-eta-0-5} ($\eta = 0.5$), are directly applicable to \tref{accuracy-eta-0} ($\eta = 0$) and \tref{accuracy-eta-1} ($\eta = 1$).
