...
$\eExp$ and $\eVar$ are chosen due to the ease of interpretation, and they are based on the analytical formulae in \eref{pc-moments}.
$\ePDF$ is chosen as it reflects the quality of the distributions estimated by our framework, and it is computed by sampling the constructed PC expansions.
...
For clarity of the discussions below, we shall primarily focus on one of the tables, namely, on \tref{accuracy-eta-0-5} (highlighted with square brackets) as the case with $\eta = 0.5$ turned out to be the most challenging (see \sref{er-speed}).
...
To give a better appreciation of the proximity of the two methods, \fref{experimental-results-pdf} displays, at time 50~ms, the \pdfs\ computed using our framework with $\pcorder = 4$ (the dashed lines) along with those calculated by the MC approach with $\nsamples = 10^4$ (the solid lines).
...
Similar to the previous observations, even for low requirements, the error of the expected values estimated by MC sampling is relatively small, namely, bounded by 1.19\% (see $\eExp$ for $\pcorder \geq 4$ and $\nsamples = 10^2$).
...
Guided by the observations in this subsection, we conclude that the proposed framework delivers accurate results starting from $\pcorder = 4$.
The estimates of the MC approach, on the other hand, can be considered as sufficiently reliable starting from $\nsamples = 10^4$.
However, the last conclusion is in favor of the MC technique since, as mentioned earlier, there is strong evidence that $10^4$ samples are still not enough.
This also conforms to the theoretical estimates of the accuracy of MC sampling given in \cite{diaz-emparanza2002} and to the experience from the literature \cite{maitre2010, shen2009, eldred2008}.
Consequently, for the experiments of the computational speed given next, we fix $\pcorder$ to four and $\nsamples$ to $10^4$.
