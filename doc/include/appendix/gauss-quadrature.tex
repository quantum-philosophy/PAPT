...
Yet another example of the computational savings delivered by the Smolyak algorithm is given in \fref{integration-grids} \cite{eldred2008} where, in two dimensions, a full-tensor product grid is plotted side by side with a sparse grid of the same integration accuracy.\footnote{This example uses a Clenshaw-Curtis quadrature rule, which is an important alternative to Gaussian quadratures; see \cite{eldred2008} for further details.}
