...
\end{equation}
where $\{ \pcb_i(\vZ(\o)) \}_{i = 1}^{\pcterms}$ is the truncated basis with $\pcterms$ polynomials of $\vars$ variables, and $\pcc{\vP}_{ki} \in \real^\cores$ are the coefficients of the expansion. The latter are computed using spectral projections as it is thoroughly explained in the appendinx, \aref{spectral-projection}. $\pcorder$ denotes the order of the expansion, which determines the maximal degree of the $\vars$-variate polynomials involved in the expansion; hence, $\pcorder$ also defines the level of accuracy. When the total-order polynomial space is used \cite{beck2011}, $\pcterms$ is
...
\end{equation}
where $k = 1, \dotsc, \steps$ and $i = 1, \dotsc, \pcterms$. Finally, \eref{pc-recurrence} and \eref{fourier-output} are combined together to compute the coefficients of the PC expansion of the temperature vector $\vTO_k(\o)$.

It can be seen that the performed spectral decompositions have replaced the recurrence in \eref{recurrence}---wherein an arbitrary functional $\vP_k(\omega)$ (see \sref{power-model}) of the uncertain parameters and stochastic temperature is involved, which, moreover, needs to be summed up in the presence of correlations with another random vector, $\vX_k(\omega)$---with a purely deterministic recurrence in \eref{pc-recurrence}, where only linear operations are used.
