...
An introduction to orthogonal polynomials is given in \aref{orthogonal-polynomials}.
...
\end{equation}
where $\{ \pcb_i(\vz) \}_{i = 1}^{\pcterms}$, $\vz \in \real^\nvars$, is the truncated basis with $\pcterms$ polynomial terms of $\nvars$ variables, and $\pcc{\vP}_{ki} \in \real^\nprocs$ are the coefficients of the expansion. The latter are computed using spectral projections as it is explained in \aref{spectral-projection}.
