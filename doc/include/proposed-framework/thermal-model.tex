...
Recall that the power profile we work with is a discrete-time representation of the power consumption, which contains $\nsteps$ power samples, or steps (see \sref{problem-formulation}).
As detailed in \aref{thermal-model}, we let the total power be constant between neighbor power steps and reduce the solution process of \eref{fourier-system} to the following recurrence, for $k = 1, \dotsc, \nsteps$,
...
Therefore, the operations involved in the recurrence given by \eref{recurrence} are to be performed on dependent random variables with arbitrary probability distributions, which, in general, have no closed-form solutions. To tackle this difficulty, we utilize PC expansions as we shall discuss in the following section.
