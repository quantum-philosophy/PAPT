...
\end{equation}
for $i \in \{ 1, \dotsc, \pcterms \}$. In general, the evaluation of the inner product in \eref{pc-coefficients}, which is defined by \eref{inner-product-integral} and \eref{inner-product-sum}, should be done numerically, and the projection becomes so-called non-intrusive. An illustrative example of this procedure for the continous case is given in \sref{pc-computation} where the multidimensional integration is involved. On the other hand, the normalization constants $\pcn_i$ are fixed for a particular polynomial basis, thus, they are precomputed and tabulated.
