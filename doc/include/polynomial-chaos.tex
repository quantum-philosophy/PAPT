...
\end{equation}
where $i,j \in \natural{0}$, $\delta_{ij}$ is the Kronecker delta function, and the operator $\oInner{\cd}{\cd}$ denotes the inner product in the Hilbert space spanned by the polynomials. The inner product with the weight function $\pdf(\vz)$ is defined as the following multi-dimensional integral:
