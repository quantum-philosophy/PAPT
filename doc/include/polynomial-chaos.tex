...
\end{equation}
when $\PDF(\vz)$ is discrete. In our case, the weight function corresponds to the probability density (or mass) function $\PDF_\vZ(\vz)$ of the r.v.'s $\vZ(\o)$, and the inner product coincides with the expectation operator. Therefore, it can be seen from \eref{orthogonality} that orthogonality is equal to the absence of correlations, and this is, in fact, what we seek for.
...
Having an appropriate basis chosen, we apply the PC procedure to the power term in \eref{ode-solution}. In this case, the truncated expansion is defined as follows:
