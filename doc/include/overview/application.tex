Before diving into details, we demonstrate a particular application of the proposed framework. Specifically, we shall perform stochastic PTA of a dual-core platform, wherein the parameters that affect the leakage current are uncertain. The demonstration is a part of the illustrative example in \sref{illustrative-example} and, therefore, will be described in details later on.
...
\]
where $\pcn_i$ are normalization constants, and the squaring should be understood element-wise. For instance, assume that the system is loaded by the nominal power profile $\profPdyn$ depicted in \fref{power}. Then, after the computations at \stage{4}, we obtain the corresponding stochastic temperature profile $\profT{\o}$ and can observe, \eg, its expectation and standard deviation; they are plotted in \fref{temperature}. The displayed curves closely match those obtained by MC simulations with $10^4$ samples; however, our method takes less than a second, on a personal computer, while the MC-based approach takes more than a day, which we shall discuss in \sref{experimental-results}.
