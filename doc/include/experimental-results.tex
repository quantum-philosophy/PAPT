...
\updated{The time step of power and temperature traces is set to $1\,\text{ms}$ (see \sref{problem-formulation}), which is also the time step of the recurrence in \eref{pc-recurrence}.}
As a comparison to our polynomial chaos (PC) expansions, we employ a Monte Carlo (MC) sampling technique.
\updated{The MC approach is set to preserve the whole variance of the problem, \ie, no model order reduction, and it solves the system in \eref{fourier-system} directly using the fourth- and fifth-order Runge-Kutta formulae (the Dormand-Prince method) \cite{press2007} available in MATLAB \cite{matlab}.}
