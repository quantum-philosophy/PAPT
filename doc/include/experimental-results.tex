...
Therefore, the columns of \tref{accuracy} corresponding to high values of $\nsamples$ can be used to assess the accuracy of PC expansions; likewise, the rows of \tref{accuracy} corresponding to high values of $\pcorder$ can be used to judge about the sufficiency of the MC samples.
Since the errors in the nine tables nested in \tref{accuracy} (three error metrics times three values of $\eta$) decrease from the top left corners to the bottom right corners, we conclude that the PC and MC methods converge.
...
\fref{experimental-results-pdf} gives an example of the \pdfs\ obtained using our framework with $\pcorder = 4$ (the dashed lines) along with those estimated by the MC approach with $\nsamples = 10^4$ (the solid lines).
...
As it was noted before with respect to \tref{speed-processing-elements}, we observe a prominent dependency of the PC-based technique on the dimensionality of $\vZ(\o)$, which is two for $\eta = 0$ and five for the other two values of $\eta$ (see \tref{speed-processing-elements} for $\nprocs = 4$).
