...
\updated{The effective channel length is assumed to have a nominal value of $45\,\text{nm}$ and a standard deviation of 5\% of the nominal value where the global and local variations are equally weighted \cite{juan2011, juan2012}.}
...
Time steps of power and temperature traces are set to $1\,\text{ms}$ (see \sref{problem-formulation}).
...
\updated{Since the temperature part of PTA is the main contribution of this work, we shall focus on the assessment of temperature profiles.
...
Note also that, since the two temperature-driven studies \cite{juan2011, juan2012} work under the steady-state assumption, and the paper \cite{juan2011}, in addition, is limited to the maximal temperature (see \sref{prior-work}), a one-to-one comparison with our framework is not possible.}
