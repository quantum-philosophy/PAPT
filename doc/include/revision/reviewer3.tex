...
\done{The motivation behind this work has been extended in Sec.~I.}
\end{authors}
...
\done{The elliptical-copula assumption of the Nataf transformation has been included in Sec.~V-A}.
...
\end{reviewer}
\begin{authors}
Thank you for the reference; it has been revised and included in the manuscript.
...
Our proposed approach eliminates the need in such manipulations.
...
At this point, one should not forget the following two facts inherent to the modeled physical phenomenon, \ie, heat transfer within electronic systems.
...
\done{The above discussion has been appended to Sec.~V-D.}
...
However, we kindly ask the reviewer to consider our response to the corresponding comment received from Reviewer 5.

We agree with the reviewer that the more uncertain parameters are taken into account, the more independent variables might eventually be needed for the construction of a sufficiently accurate PC expansion (see our answer to the next comment of the current reviewer).
...
In response to the first comment of Reviewer 4, we have also included an analysis of the time complexity of the PC expansions constructed inside the proposed framework in Sec.~V-D4.

\done{The curse of dimensionality has been further elaborated on in Sec.~V-D4.}
\end{authors}
...
\end{reviewer}
\begin{authors}
Let us please note the following two things.
...
Second, the conclusions about the performance the proposed framework, which are based on the number/essence of the considered sources of variations $\vU(\o)$, can be misleading as we would like to illustrate now.
...
As with the previous comment of the current reviewer, our answer to the comment of Reviewer~5 regarding the considered uncertain parameters is relevant to the current comment.
