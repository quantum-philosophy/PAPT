...
In response to the first comment of Reviewer 4, we have also included an analysis of the time complexity of the PC expansions performed inside our proposed framework, Sec.~V-D4.

With regard to the choice of the unique uncertain parameter for the illustration example, the reviewer might also read our answer to the fourth comment of Reviewer 5.
...
Second, the conclusions about the performance of the proposed framework, which are based on the number/nature of the considered sources of variations $\vU(\o)$, can be misleading as we would like to illustrate now.
...
As with the previous comment, our answer to the fourth comment of Reviewer~5 regarding the considered uncertain parameters is relevant to the current comment.

\begin{actions}
  \action{The above discussion in connection with Tab.~V has been included in Sec.~VII-B.}
\end{actions}
\end{authors}
