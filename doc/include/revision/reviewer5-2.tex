...
Therefore, under the assumption that the problem is well posed, as our iterative procedure advances, the increasing powers of $\mCF$ does not allow the numerical solution to explode [Hochbruck, 2010], which one can expect from $\hat{\mCF} = \mI + \Delta \t \mA$ with an inadequately chosen time step.

Consequently, the time step used in the recurrence given by Eq.~(3) does not impose limitations on the proposed framework.
...
The accuracy of the recurrence in Eq.~(3) can be improved using more advanced exponential iterators, for example, linear multistep exponential iterators; however, we did not explore this direction.
...
There are two main aspects in this regard, which are described in Sec.~VII.
