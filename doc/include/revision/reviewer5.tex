...
\end{reviewer}
\begin{authors}
Thank you for your critique. Please find our response below.
\end{authors}
...
\end{reviewer}
\begin{authors}
Both PC and PCA/KL are tools, and tools are supposed to be used to solve certain problems.
...
In particular, the user does not have to come up with any expressions describing the joint effects of the uncertain parameters and to perform curve fitting procedures (\eg, to the data from SPICE simulations) in order to determine the coefficients of the chosen expressions.
...
As mentioned in Sec.~VI-B, in our experiments, we simply utilize linear interpolation without any polynomial-based fitting, which the proposed framework readily allows us to do.
...
On the other hand, [Ghanta, 2006] is founded on the basis of the \emph{continuous} version of the KL decomposition and, therefore, assumes that the correlation function of the uncertain parameters is known.
...
On the other hand, [Ghanta, 2006] is tightened to Gaussian distributions, which are critical for the development in that paper.
In contrast,
\begin{enumerate}
  \setcounter{enumi}{4}
...
We sincerely hope that our work will give birth to other temperature-related studies where the presence of uncertainty will be seamlessly taken into account at little effort.
...
\begin{reviewer}
Also for multi-core and many-core architectures, the temperatures are highly loads dependent and the variations due to process variations maybe still secondary effects for a chip designers. Temperature is also environment and boundary conditions (fan speeds, liquid speed) dependent
\end{reviewer}
\begin{authors}
