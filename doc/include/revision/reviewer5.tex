...
In particular, \feature{2} implies the user does not have to come up with artificial expressions describing the joint effects of the uncertain parameters and to perform curve fitting procedures to the data from SPICE simulations (see, \eg, [Bhardwaj, 2008]) in order to determine the coefficients of these expressions.
...
In order to illustrate this concern, we would like to include here the following two paragraphs taken from our response to the seventh comment of Reviewer 3.
...
As we can see, it is not straightforward to relate $\vU(\o)$ with the dimensionality $\nvars$ of $\vZ(\o)$ and, thus, with the complexity of the further analysis.
...
\begin{actions}
  \action{The applicability of our approach has been discussed in connection with Tab.~V in Sec.~VII-B, which also addresses the seventh comment of Reviewer 3.}
\end{actions}
\end{authors}
...
\begin{actions}
  \action{The formulation in the abstract has been clarified.}
  \action{Possible solutions of general distributions have been outlined in App.~C.}
\end{actions}
\end{authors}
...
\end{reviewer}
\begin{authors}
As mentioned in our previous answers, the stochastic power-temperature analysis presented in the paper does take into consideration the interdependence between leakage and temperature.
...
\begin{actions}
  ...
  \action{The negligence of the interdependence by the prior works has been emphasized in Sec.~II.}
...
For a better intuition of the computational speed of the proposed framework, we kindly refer the reviewer to our answer to the first comment of Reviewer 4 wherein an analysis of the time complexity is given.
