...
The user is required: (a) to provide a thermal specification of the platform $\system$; (b) to have prior knowledge (or belief) on the probability distribution of the uncertain parameters (\sref{uncertain-parameters}); and (c) to specify a power model, in which $\vU(\o)$ is an input (\sref{power-model}).
