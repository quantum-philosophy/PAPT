...
\updated{The system is assumed to depend on a set parameters, which are uncertain at the design stage; these parameters are denoted by a random vector $\vU(\o)$, $\o \in \outcomes$.
Once the fabrication process yields a particular outcome $\o$, $\vU(\o)$ take (potentially) different values for each fabricated chip individually and stay unchanged hereafter.
...
\updated{The goal of this work is to develop a system-level probabilistic framework for power-temperature analysis (PTA) of electronic systems where the actual power dissipation and temperature are stochastic due to their dependency on the set of uncertain parameters $\vU(\o)$.}
