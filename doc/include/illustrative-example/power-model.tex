...
At Stage~3, the equivalent thermal RC circuits---essentially, the coefficient matrices in \eref{fourier-system}---are constructed by means of HotSpot v5.02 \cite{hotspot}. Thermal packages are modeled with three layers, and the relation between the number of processing elements and the number of thermal nodes is given by $\nodes = 4 \cores + 12$. An example of such a circuit is depicted in \fref{circuit}, and additional details are given in \aref{thermal-model}.
