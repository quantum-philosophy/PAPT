function scaleProcessors
  ...
  experimentCount = length(processorCountSet);

  chaosOptions = Options('order', 4, ...
    'quadratureOptions', Options( ...
      'ruleName', 'GaussHermiteHW', 'method', 'adaptive', 'polynomialOrder', 4));
  ...
  line(processorCountSet, measurements(:, 1), ...
    'Color', Color.pick(1), 'Marker', 'o')
  line(processorCountSet, measurements(:, 2), ...
    'Color', Color.pick(2), 'Marker', 'x')
  line(processorCountSet, measurements(:, 3), ...
    'Color', Color.pick(3), 'Marker', 's')

  set(gca, 'YScale', 'log');

  Plot.title('Scaling with the number of processing elements');
  Plot.label('Number of processing elements', 'log(Time, s)');
